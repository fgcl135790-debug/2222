...
class IntradayProfitModel:
    ...
    def _professional_calibration(self, result, feature, action, required_win_rate, min_expected_value):
        ...
        if sample_count < 35:
            penalty += 10
            filters.append("樣本少於 35 筆，勝率折扣 10%")
        elif sample_count < 60:
            penalty += 5
            filters.append("樣本少於 60 筆，勝率折扣 5%")
        if self.training_days < 10:
            penalty += 8
            filters.append("訓練日少於 10 日，勝率折扣 8%")
        elif self.training_days < 15:
            penalty += 4
            filters.append("訓練日少於 15 日，勝率折扣 4%")
        if level in ["ALL", "TIME_ONLY"]:
            penalty += 5 if level == "ALL" else 3
        ...
        if minute >= 210:
            penalty += 12
            filters.append("12:30 後新倉，停利空間通常不足，強制提高門檻")
        ...
        if action == "BUY":
            ...
            if orb_high_gap >= 0 and vwap_gap >= 0 and volume5 >= 0.85:
                setup_type = "ORB突破做多"
                ...
                penalty += 9
                filters.append("做多未通過 ORB / VWAP / 動能主要濾網")
                if minute < 120:
                    blocks.append("早盤做多方向未與 ORB 或 VWAP 對齊")
            ...
            vwap_ok = vwap_gap <= 0.05
            momentum_ok = slope3 <= 0.10 and slope10 <= 0.25
            if orb_low_gap <= 0 and vwap_gap <= 0 and volume5 >= 0.85:
                setup_type = "ORB跌破做空"
                ...
                penalty += 9
                filters.append("做空未通過 ORB / VWAP / 動能主要濾網")
                if minute < 120:
                    blocks.append("早盤做空方向未與 ORB 或 VWAP 對齊")
        ...
        calibrated_ev = raw_ev - penalty * 0.018
        ...
        if minute >= 210 and not (calibrated_win_rate >= required_win_rate + 16 and calibrated_ev >= min_expected_value + 0.35 and volume_acc >= 1.4):
            hard_fail = True
            hard_reasons.append("12:30 後除非極強訊號，否則不開新倉")
        if minute >= 150 and not (calibrated_win_rate >= required_win_rate + 10 and calibrated_ev >= min_expected_value + 0.20):
            hard_fail = True
            hard_reasons.append("11:30 後需要更高勝率與期望值")
        if action == "BUY" and vwap_gap > 2.0:
            hard_fail = True
            hard_reasons.append("做多離 VWAP 超過 2%，禁止追高")
        if action == "SELL" and vwap_gap < -2.0:
            hard_fail = True
            hard_reasons.append("做空離 VWAP 超過 2%，禁止追空")
    ...
    def predict(
        ...
        if decision == "WAIT":
            ...
            score = max(0, min(78, int(42 + max(buy_edge, sell_edge) * 0.45)))
        else:
            risk_level = "NORMAL"
